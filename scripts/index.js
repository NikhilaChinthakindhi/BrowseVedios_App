$(document).ready(function () {
    ...
    //localStorage.setItem("vedio_app_users", JSON.stringify(localObject));
    //console.log(JSON.parse(localStorage.getItem("vedio_app_users")));


    //user registration
    $("#reg_submit").click(function () {
        if (typeof (Storage) !== "undefined") {
            var existing_users = JSON.parse(localStorage.getItem('vedio_app_users'));
            ...
            localStorage.setItem("vedio_app_users", JSON.stringify(existing_users));
            console.log(JSON.parse(localStorage.getItem("vedio_app_users")));
    ...
    $('#login_submit').click(function () {
        ...
        var existing_users_vedio_app = JSON.parse(localStorage.getItem('vedio_app_users'));
        for (i = 0; i < existing_users_vedio_app.length; i++) {
            if (username_input == existing_users_vedio_app[i].userName && password_input == existing_users_vedio_app[i].password) {
                matchFlag = 1;
                userID = existing_users_vedio_app[i].user_id;
            }
        }

        if (matchFlag == 1) {
            window.location.href = "../vedios.html?" + userID;
        }
        else {
